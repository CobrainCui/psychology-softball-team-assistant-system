...
export function emptySkillTableSlice(): SkillTableSlice {
  return {
...
export function useSkillTableDraft(onNotice?: (message: string) => void) {
  ...
  const hydrate = (slice: SkillTableSlice) => {
    setSpeedColumns(slice.speedColumns);
    setSpeedMarks(slice.speedMarks);
    setFlyCatchAttempts(slice.flyCatchAttempts);
    setStrikeJudgeColumns(slice.strikeJudgeColumns);
    setStrikeJudgeCells(slice.strikeJudgeCells);
    setThrowPlays(slice.throwPlays);
    setFlyCatchNoteDrafts({});
    setEditingFlyCatchId(null);
  };

  const reset = () => {
    hydrate(emptySkillTableSlice());
  };
