...
function testPolicyMatrix() {
  ...
  const adminOnly = ctx({ roles: ["player", "admin"], playerId: null, isApproved: false, claimStatus: "pending" });
  if (canViewTeamHealth(adminOnly)) fail("admin-only cannot view health");
  else pass("admin-only cannot view health");
  if (canManageAccounts(adminOnly)) fail("pending admin cannot manage accounts");
  else pass("pending admin cannot manage accounts");
  const approvedAdmin = ctx({ roles: ["player", "admin"] });
  if (canManageAccounts(approvedAdmin)) pass("approved admin can manage accounts");
  else fail("approved admin can manage accounts");
